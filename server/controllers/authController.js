...
exports.register = async (req, res, next) => {
  try {
    ...
    const user = { id: result.insertId, email, role, first_name: firstName, last_name: lastName };
    res.status(201).json({ success: true, data: { ...user, token: generateToken(user.id) } });
...
exports.login = async (req, res, next) => {
  try {
    ...
    const userData = { id: user.user_id, email: user.email, role: user.role, first_name: user.first_name, last_name: user.last_name, avatar_url: user.avatar_url };
    res.json({ success: true, data: { ...userData, token: generateToken(user.user_id) } });
  } catch (error) { next(error); }
};
